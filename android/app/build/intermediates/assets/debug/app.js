document.addEventListener('DOMContentLoaded', async () => {
        ...
        try {
            ...
            try {
                ...
                if (permissionStatus.state === "denied") {
                    showPermissionModal('Camera permission denied. Please enable camera access in your browser settings to scan QR codes.', 'error');
                    stopQrScanner();
                    return;
                }
            } catch (permError) {
                console.log('Permission query not supported, continuing anyway:', permError);
            }
            ...
            if (err.name === 'NotAllowedError' || err.name === 'PermissionDeniedError') {
                showPermissionModal('Camera permission denied. Please allow camera access to scan QR codes.', 'error');
            } else if (err.name === 'NotFoundError' || err.name === 'DevicesNotFoundError') {
                showPermissionModal('No camera found on this device.', 'error');
            } else if (err.name === 'NotReadableError' || err.name === 'TrackStartError') {
                showPermissionModal('Camera is already in use by another application.', 'error');
            } else {
                showPermissionModal('Camera access error: ' + err.message, 'error');
            }
    ...
    async function startNfcScan() {
        ...
        if (typeof AndroidNFC !== 'undefined') {
            if (!AndroidNFC.isNFCAvailable()) {
                showPermissionModal('NFC not available on this device.', 'error');
                return;
            }
            
            if (!AndroidNFC.isNFCEnabled()) {
                showPermissionModal('NFC is disabled. Please enable NFC in device settings.', 'error');
                setTimeout(() => {
            ...
            // Open NFC dialog and start scanning
            openNfcDialog();
            isNfcScanning = true;
            showNfcScanning();
            console.log('Native NFC scanning started');
            return;
        }
        
        // Fallback to Web NFC API (for browsers that support it)
        if (!('NDEFReader' in window)) {
            showPermissionModal('NFC not supported on this device. Please use a device with NFC capability.', 'error');
            return;
        }

        // Check NFC permission first
        try {
            const permissionStatus = await navigator.permissions.query({ name: "nfc" });
            
            if (permissionStatus.state === "denied") {
                showPermissionModal('NFC permission denied. Please enable NFC in your browser settings and allow NFC access for this site.', 'error');
                return;
            }
        } catch (permError) {
            console.log('Permission query not supported, continuing anyway:', permError);
        ...
        try {
            ...
            if (error.name === 'NotAllowedError') {
                showPermissionModal('NFC permission denied. Please enable NFC access in your device settings and browser permissions.', 'error');
            } else if (error.name === 'NotSupportedError') {
                showPermissionModal('NFC is not supported or disabled on this device.', 'error');
            } else {
    ...
    exportExcelBtn.addEventListener('click', () => {
        ...
        if (exportOption === 'current') {
            ...
            console.log('Generated current session CSV, length:', csv.length);
            ...
            if (csv) {
                ...
                console.error('Failed to generate subject CSV');
                alert('Failed to generate CSV for selected subject.');
            }
        }
    });
    ...
    async function generateCurrentSessionPDF() {
        ...
        try {
            ...
            let logoImage = null;
            try {
                if (typeof Android !== 'undefined' && Android.getLogoBase64) {
                    // For Android, get logo as base64 from native interface
                    const logoBase64 = Android.getLogoBase64();
                    if (logoBase64) {
                        const logoBytes = Uint8Array.from(atob(logoBase64), c => c.charCodeAt(0));
                        logoImage = await pdfDoc.embedPng(logoBytes);
                    }
                } else {
                    // For browser, use fetch with relative path
            ...
            const filename = `USM_Attendance_${classLabel}_${new Date().toISOString().slice(0, 10)}.pdf`;
            
            // Check if running in Android WebView
            if (typeof Android !== 'undefined' && Android.downloadFile) {
                // Convert PDF bytes to Base64 for Android
                let binary = '';
                ...
                Android.downloadFile(base64, filename, 'application/pdf');
            } else {
                // Browser download
    ...
    // Global handler for native Android NFC tags
    // Store existing handler if login.js already installed one
    const existingNFCHandler = window.handleNFCTag;
    
    window.handleNFCTag = async function(tagId) {
        console.log('Native NFC tag detected:', tagId);
        
        // Check if we're on the login page (NFC modal exists and is active)
        const loginNfcModal = document.getElementById('nfc-modal');
        if (loginNfcModal && loginNfcModal.classList.contains('active')) {
            console.log('Login page detected - delegating to login handler');
            if (existingNFCHandler && typeof existingNFCHandler === 'function') {
                return await existingNFCHandler(tagId);
            }
        }
        
        if (!isNfcScanning) {
            console.log('NFC scanning is not active');
            return;
        }
        
        nfcScanCount++;
        updateNfcScanCount();
        await processNFCScan(tagId);
    };
    
    function downloadCSV(csv, filename) {
        // Check if running in Android WebView
        if (typeof Android !== 'undefined' && Android.downloadFile) {
            // For Android, encode CSV to handle special characters properly
            try {
                Android.downloadFile(csv, filename, 'text/csv');
                console.log('CSV download initiated via Android interface');
            } catch (error) {
                console.error('Android CSV download error:', error);
